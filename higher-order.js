...
//Code Here
let totalPopulation = populations.reduce((pop, pop2) => pop + pop2);
...
let ordersTotal = orders.map((order) => order.price + order.tax).reduce((first, rest) => first + rest);
...
let bobsTotal = purchases.filter((bob) => bob.owner === "Bob").map((bob) => bob.price).reduce((total,rest) => total+rest);
